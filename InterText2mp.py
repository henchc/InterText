...
"""This program takes ngrams of two texts, compares using fuzzywuzzy,
and yields intertextualities. Input is text and folder of texts to be compared,
window of n-grams, and threshold for match 0 - 100."""
...
def process_ngrams(i, ctri):
    ...
    if cscore < sens:
        for j, mtri in enumerate(mtril):
            if fuzz.token_sort_ratio(ctri, mtri) > thresh and i >= window and
            j >= window and i < (len(ctril) - window)
            and j < (len(mtril) - window):

                # match grabs n-gram size on each side, for later csv writing
                match = ((ctril[i - (window)] + " " + ctri + " " + ctril[i +
                         (window)]),
                         (mtril[j - (window)] + " " + mtri + " " + mtril[j +
                          (window)]))
...
for textb in glob.glob(folder + '/*'):
    ...
    matchdata = Parallel(n_jobs=num_core)(
        delayed(process_ngrams)(i, ctri) for i, ctri in enumerate(ctril))
    ...
    for i, item in enumerate(newmatches):
        ...
        if pos != -1:
            ntext1 = ntext1[:pos] + '<a name="intta' + str(i) +
            '"></a><h4 style="color:red;">(' + str(i) + ") " + ntext1[
                pos:pos + len(item[0])] + '<a href="#inttb' + str(i) + '"> (' +
            f2name + ')</a></h4>' + ntext1[pos + len(item[0]):]

        pos = ntext2.find(item[2])
        if pos != -1:
            ntext2 = ntext2[:pos] + '<a name="inttb' + str(i) +
            '"></a><h4 style="color:red;">(' + str(i) + ") " + ntext2[
                pos:pos + len(item[2])] + '<a href="#intta' + str(i) +
            '"> (' + f1name + ')</a></h4>' + ntext2[pos + len(item[2]):]
